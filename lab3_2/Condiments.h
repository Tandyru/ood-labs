...
{
public:
	static const unsigned MAX_SLICE_COUNT = 5;

	CChocolate(IBeveragePtr && beverage, unsigned sliceCount)
		: CCondimentDecorator(move(beverage))
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return "Chocolate slices x " + std::to_string(m_sliceCount);
	}
private:
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		auto type = (m_type == Type::Nut) ? "Nut" : "Chocolate";
		return std::string(type) + " liquor";
	}
private:
	Type m_type;
};